...
function contextMenuClick(args: any) {
  ...
  if (diagram?.selectedItems?.nodes?.length > 0) {
    // @ts-ignore
    const bpmnShape: any = diagram?.selectedItems?.nodes[0].shape
    if (bpmnShape) {
      if (args.item.iconCss.indexOf('e-adhocs') > -1) {
        bpmnShape.activity.subProcess.adhoc = args.item.id !== 'AdhocNone'
      }
      ...
      if (args.item.iconCss.indexOf('e-loop') > -1) {
        const loop = args.item.id === 'LoopNone' ? 'None' : args.item.id
      ...
      if (args.item.iconCss.indexOf('e-compensation') > -1) {
        const compensation = args.item.id !== 'CompensationNone'
        if (bpmnShape.activity.activity === 'Task') {
      ...
      if (args.item.iconCss.indexOf('e-call') > -1) {
        const compensation = args.item.id !== 'CallNone'
        if (bpmnShape.activity.activity === 'Task') {
      ...
      <div className="control-pane">
        <div className="control-section">
          ...
          <div id="diagram-space" className="sb-mobile-diagram sb-bpmn-editor">
            <DiagramComponent
              id="diagram"
              ref={(diagramObj) => (diagramInstance = diagramObj)}
              width="100%"
              height="445px"
